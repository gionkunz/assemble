...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('assemble', 'Compile template files with specified engines', function() {
    ...
    var optionsConfiguration = function(assemble, next) {
      ...
      assemble.layoutFiles = file.expand({filter: 'isFile'}, assemble.options.layoutdir || assemble.options.layouts);
      if(assemble.layoutFiles.length === 0) {
        assemble.layoutFiles = null;
      } else {
        grunt.verbose.ok(">> Layout files:".yellow, assemble.layoutFiles);
      }
